...
local function make_item_filter(search)
    local needle <const> = string.lower(search or "")
...
local function compare_socket_size(socket_a, socket_b)
    local a <const>, b <const> = SOCKET_SIZES[socket_a], SOCKET_SIZES[socket_b]
...
local function numformat(value, precision)
    ...
    if precision == nil then
        ...
        local p <const> = 10 ^ precision

        local i <const> = math.floor(value)
